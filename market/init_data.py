...
def update_user_budget(app, user_id, new_budget):
    with app.app_context():
        user = User.query.get(user_id)  # Fetch the user by ID
        if user:
            user.budget = new_budget  # Update the budget
            db.session.commit()  # Commit the changes to the database
            print(f"User {user_id} budget updated to {new_budget}")
        else:
